...
describe("Тестування формы создания задачи", () => {
  ...
  it("При помилці валідації, показується error message", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Пріоритет"), {
      target: { value: "low" },
    });
    fireEvent.change(screen.getByLabelText("Дата виконання"), {
      target: { value: formattedPastDate },
    });

    fireEvent.click(screen.getByText("Створити задачу"));

    expect(
      await screen.findByText("Заголовок має бути більше 5 символів"),
    ).toBeInTheDocument();
  });
  ...
  it("Кнопка доступна, якщо валідація пройдена", () => {
    ...
    fireEvent.change(screen.getByLabelText("Опис (Не обовʼязково)"), {
      target: { value: "test" },
    });
    fireEvent.change(screen.getByLabelText("Статус"), {
