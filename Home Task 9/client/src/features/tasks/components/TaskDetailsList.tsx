...
const TaskDetailsList = ({ task, user }: TaskDetailsListProps) => {
  return (
    <>
      <h2 className="taskDetailsTitle">{task.title}</h2>
      <p className="taskDetailsInfo"> Виконавець: {user}</p>
      ...
      <p className="taskDetailsInfo">
        Дата створення: {dateFormat(task.createdAt)}
      </p>
      <p className="taskDetailsInfo">
        Дата виконання: {dateFormat(task.deadline)}
      </p>
    </>
  );
};
